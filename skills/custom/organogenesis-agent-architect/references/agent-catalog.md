...
**Retrieval-audit application (NEW, ADR-0022 — answer-pipeline loop).** A specialization of Mode 1 (Self-Consistency mandatory) for auditing evidence the answer-pipeline retrieved — from the DATA INAMOVIBLE OR an external source (Tool Universe / Europe PMC) — *before* it is used in an answer or proposed for ingestion. **≥3 adversarial auditors** (default-to-REJECT), each with a distinct lens (*relevance/correctness* · *novelty + anti-fabrication* · *hard skeptic*), independently verify, per item: **(a) absence re-check** — is the fact genuinely NOT already in the DI (`resolve_id` + a negated query)? — and **(b) veracity** — does the source actually support the claim (read the cached full text) and do all identifiers resolve (no hallucination, §7.9)? Verdict APPROVE/REJECT per item; majority decides. Only APPROVED evidence flows to the answer; APPROVED + DI-worthy → `propose_from_external.py` (human gate). It is wired into `answer_pipeline.record_audit()`: the decision-state machine **blocks answering external evidence until this verdict is recorded** (the path is structural, not contract-dependent). Demonstrated 2026-06-13: 3/3 REJECT of two off-target pronephros papers, and the auditors surfaced the genuinely on-point source (Gerlach & Wingert 2014) from a bibliography citation.
