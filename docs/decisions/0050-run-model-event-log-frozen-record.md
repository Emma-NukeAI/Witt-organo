...
**Primera corrida REAL en producción (2026-08-10, Emmanuel):** run `a361f566d67f470eb195c78b2b3cb7b6` — *"Is wt1a required for zebrafish pronephros development?"* (entities wt1a, pax2a) — ejecutada en el `witt-query-service` desplegado (Dokploy interno, panel live Opus+Sonnet+Haiku+gpt-4o con spend real): `queued → running → awaiting_closure → closed`, **`frozen_at: 2026-08-10T03:12:15+00:00`, `closed_by: emmanuel`**. Es el primer registro congelado y cerrado de la historia del sistema — la unidad exacta que la webapp renderiza.
