...
| [0080](0080-competencia-y-harness-de-busqueda.md) | 2026-09-15 | La compuerta de competencia y el harness de búsqueda: la decisión "¿basta la pasada 1 o se busca afuera?" la toma CÓDIGO (conjunción de componentes deterministas; la confianza del modelo se mide y viaja pero NO decide — `cg-2`, gateable sólo por env; kill-switch declarado que devuelve la Ruta B a ADR-0078 con `trigger 'confidence'`), la Ruta B corre por un harness de familias Layer 0 (`SEARCH_DISPATCH` de 15 familias sobre 10 tools stdlib nuevas, rondas con presupuesto, ledger por fuente con estados), la escalera de soporte por cita (unresolved → resolved → passage_delivered → supported|unsupported, nunca fundida), afirmación positiva sin citas = inadmisible, reintento por juez declarado, gasto por etapa (`by_stage` cuadra con `by_model`), contrato 1.9 | Accepted |
| [0081](0081-politica-best-tier-v2-y-generacion-de-modelos-g2.md) | 2026-09-15 | Política best-tier v2 y generación de modelos `g2-2026-09`: UNA tabla de modelos como única verdad (`analysis/scripts/lib/models.py`, dos generaciones; kill-switch `WITT_MODEL_GENERATION=g1-2026-08` = f57a3d3 byte a byte), el modelo que CORRIÓ se MIDE y no se copia (`frozen.models.ran` requested/reported/relation; evento `stage.models` antes de gastar), el juez OpenAI habla la Responses API con vocabulario CERRADO de fallos (`error_kind`; `gpt-4o` sigue como puente hasta el smoke en vivo con Astra), el cuórum exige ≥ 2 FAMILIAS y ≥ 3 lentes (un panel de una sola familia JAMÁS aprueba: REVISE estructural con `panel_incomplete_reasons`), Opus 5 piensa por default (topes g2, `WITT_ANTHROPIC_EFFORT`), retiro de haiku declarado con aviso 30 días y sucesor en tabla, `root_run_no` desde la BD, `GET /threads`, `/usage` por etapa, bitácora `config_history` (archivo ATESTIGUADO + tabla MEDIDA), contrato 1.10; supersede la cláusula "≥ 1 Fable en el panel" de ADR-0031 | Proposed |
| [0082](0082-consejo-de-criterio-ejecutable.md) | 2026-09-15 | El consejo de criterio ejecutable: las 31 fichas del catálogo se parsean como CÓDIGO (`catalog_cards`, sha por ficha + `CATALOG_SHA` que viaja en cada corrida) y 17 de ellas se sientan por TABLA (`agent_matrix` v1.3, membresía `cm-1`; 8 operativos `not-applicable-by-category`, 9 de sustrato con estado; 19/34 filas ejecutables, antes 2/29); la ronda 1 (requisitos de información) es un JOB del PLAN que un humano aprueba en un ledger (keep / discard con razón / "yo lo aporto" atestiguado — `causal-pruner` exige decisión EXPLÍCITA, §7.1) ANTES de que la corrida gaste; la ronda 2 mide COBERTURA de cada requisito sobre `DI + path_b` ANTES de la compuerta de competencia y `council_uncovered_must` pasa a componente GATEANTE (`cg-4`); sus directivas COMPILADAS por código mueven la búsqueda dirigida en UNIÓN con las 5 familias auto; la agregación es código byte a byte; el consejo JAMÁS escribe respuesta, veredicto, ranking ni despacha; modelo `claude-opus-5` por la tabla g2 (rol `council`), prefijo compartido cacheado y cotizado, semáforo de proceso para toda llamada Anthropic; 8 rutas nuevas (`/plans/*`, `/council/*`), `plan_events`, `council_index`; kill-switch `WITT_COUNCIL=0` = camino `9d90c01` con excepciones declaradas; contrato 1.11, `plan_version 4` | Proposed |
| [0083](0083-figuras-como-evidencia-observada-y-pdf-completo.md) | 2026-09-15 | Figuras de papers como evidencia OBSERVADA: el `<fig>` del XML JATS que `fetch_paper` ya cachea se parsea (`lib/figures.py`, `jats-fig-1`), sus bytes se bajan UNA vez por paper del zip `supplementaryFiles` de Europe PMC y son MEDICIÓN sólo como source-pointer (fig_id + sha256 recalculado al gatear, servir y embeber + licencia por tabla CERRADA `lt-1`: CC BY/CC0/CC BY-SA embebibles, NC/ND sólo caption + enlace, `unknown` jamás se embebe, ZFIN jamás bytes); lo que la imagen DICE es JUICIO de exactamente dos lentes del panel (`figure_readings`, `model-judgment`; regla literal «never derive numbers from the image»), el sintetizador recibe caption + metadatos y JAMÁS bytes; cinco predicados deterministas en `verify_output` (`figure_id_resolves`, `figure_sha_matches` duro sólo en MISMATCH, `figure_only_not_asserted` — la regla §7 que no existía —, dos informativos); bytes FUERA del registro (ADR-0074) en `mcp_cache/figures/` servidos por `GET /runs/{id}/figures/{sha256}` (403 por licencia, 409 si el sha no cuadra); etapa propia `stage.figures` con presupuesto y latidos; `record_pdf.py` re-estructurado por TABLA (`SECCIONES`, `KEY_BORN`): cada llave top-level del registro con sección espejo y TRES estados con el contrato de nacimiento CALCULADO, gate de cobertura que falla si una llave del frozen real no tiene sección (las 23 líneas `[pdf]` de la paridad se saldan); kill-switch `WITT_FIGURES=0` = frozen 1.11 byte a byte salvo EXACTAMENTE 3 excepciones; smokes 100 % offline (2 XML golden + zip CC BY reducido + zip NC SINTÉTICO); contrato 1.12 | Proposed |
| [0084](0084-busqueda-libre-como-localizador.md) | 2026-09-16 | Búsqueda libre en internet como LOCALIZADOR, jamás fuente: Brave Search REST sin modelo como primario (plan «Search», SÓLO parámetros documentados, raw íntegro en disco y corte de `description` en la SALIDA) y Anthropic `web_search_20250305` como alterno EXPLÍCITO sin failover (rol `elicitation` por env, sin `tool_choice`; declara que el despachador LEE texto web — cableado, NO operativo hasta LG5), resolutor DETERMINISTA URL→identificador por TABLA cerrada de 12 reglas por host (`lib/web_locator.py`, `wl-1`/`wlr-2`; regla DOI genérica `pattern-only` encendida), materialización en la MISMA ronda SÓLO por Europe PMC (`fetch_paper._resolve_one`, 1 GET sin escribir caché; **0 ítems `source 'web'` por construcción** — los web-localizados entran como `source 'europepmc'` / `source_family 'web'` / `identifier_provenance 'web-located:<regla>'`), web PRIMERA en la ronda SÓLO cuando entra por directiva, admisión native-first y tie-break `native-before-web-located` (plan y selección sin web byte-idénticos), cuatro predicados en `verify_output` (3 duros; `web_text_not_cited` con carve-out `https://doi.org/`), disponibilidad DINÁMICA `search_harness.family_available` (con llave el consejo COMPILA la directiva y recomputa `harness_state`; `DEMAND_FAMILIES` estático para que `/council/demand` siga midiendo la demanda), cuota mensual en tabla `web_locator_usage` con UPDATE condicional atómico (`WITT_WEB_MONTHLY_CAP=900`, `skipped-cap` + detail, sin literal nuevo), costo APARTE (`token_usage.web_locator`, `/usage.web_locator` con `month_to_date`; `estimated_cost_usd` INTACTO + `estimated_cost_usd_total_projected` con clase), `frozen.web_locator` SIEMPRE presente en ≥ 1.13 (las URLs viven SÓLO ahí — jamás en eventos, prompt, panel, consejo ni `answer.gap_flags`, que recibe a lo sumo 2 strings de CONTEO), evento `stage.web.locate` por consulta, sección 53 del PDF (`localizador`), 17 env `adr '0084'` en `ENV_TABLE` + `BRAVE_API_KEY` sólo presencia («never git»), sin llave → `tool-unavailable` con CERO red; kill-switch `WITT_WEB_LOCATOR=off` (o sin llave = `off` DERIVADO) = frozen 1.12 byte a byte salvo EXACTAMENTE 3 excepciones para el mismo fixture sin datos web (los literales del consejo son los de 1.12; lo que sólo existe con datos web está enumerado en `runs.WEB_ADDITIVE_KEYS_WITH_WEB_DATA`; web nombrada en la env bajo off deja el plan y la fila MÍNIMA de 7d9ce15) y el literal de exclusión `unsatisfiable-by-harness (tool-unavailable (ADR-0084))` byte-idéntico; corrector 2026-09-16: texto RETENIDO del paper web-localizado no entregado por `fetch_external`, dedup por PAPER en la familia web, tipo de server-tool con vocabulario cerrado, materialización con timeout acotado y `record mismatch`; precedente agéntico 2026-05-14 declarado no admisible y medido no leído; smokes 100 % offline con fixtures SINTÉTICOS declarados (sin llave real hoy: LG0–LG9 los corre Emmanuel); 4 smokes nuevos + 9 tocados, 45/45 exit 0 (MEDIDO W7 2026-09-16); contrato 1.13 | Proposed |

| [0086](0086-imagenes-atestiguadas-del-laboratorio.md) | 2026-09-21 | Imágenes que aporta una PERSONA como PRIOR ART ATESTIGUADO, jamás evidencia: biblioteca propia (`lib/attestations.py`, stdlib pura) que valida por MAGIC BYTES (el `Content-Type` declarado se registra y no decide), borra metadatos SIN recodificar (JPEG/PNG/WebP/GIF walkers; si el borrado no se puede garantizar, 422 y nada se guarda), identifica por sha256 de los bytes ALMACENADOS y guarda en un almacén PRIVADO intercambiable (local 0o700/0o600 | MinIO en bucket dedicado; jamás presigned, jamás fallback silencioso: 503 declarado); tabla `plan_attested_images` por `create_all` (CERO ALTER) con identidad y procedencia y ningún byte; **subir sólo PREPARA** — el ledger humano es lo que adjunta (write-once, `attached_by_is_uploader` declarado porque quien aprueba puede no ser quien subió) y un plan sellado a una corrida no recibe más; 7 puertas HTTP (subida multipart con 413 por Content-Length ANTES de leer el cuerpo, herencia que copia bytes conservando la procedencia original, dos índices sin bytes con `viewer_may_view` calculado en el SERVIDOR, bytes con sha RECALCULADO al salir y `Cache-Control: private, no-store`, retiro como LÁPIDA con cascada que funciona bajo kill-switch); dos predicados DUROS en `verify_output` (citar una imagen aportada, por id o por sha, vuelve inadmisible la respuesta; su sha entre los identificadores de evidencia también) más uno informativo; los BYTES sólo a <= 2 lentes con visión, rotulados y separados de las figuras, con presupuesto b64 COMPARTIDO, y lo que una lente lee sólo en `attested_readings` (clase `model-judgment`); el sintetizador y el consejo reciben pie de foto + metadatos y JAMÁS un píxel, y una imagen no cubre un requisito por sí sola; `frozen.attested_images` SIEMPRE presente en >= 1.14 con sus cuatro estados (hubo | 0 MEDIDO | sin canal | apagado), sección 54 del PDF SIN miniaturas jamás y sin el pie de foto de material de paciente; autorización author-only por defecto, `team` sólo declarado + `WITT_ATTESTED_TEAM_VIEW=1`, y material de paciente author-only SIEMPRE; 18 env `WITT_ATTESTED_*` (15 en `ENV_TABLE`; ruta y credenciales FUERA: sólo su presencia) + volumen persistente OBLIGATORIO; kill-switch `WITT_ATTESTED_IMAGES=0` = frozen 1.13 byte a byte salvo EXACTAMENTE 3 excepciones, con el RETIRO vivo; cuatro correctores que los gates destaparon (el interruptor de equipo que no cerraba nada, la compuerta de citas inerte con imágenes adjuntas, un `n_readings` estructuralmente 0 porque el tool no tenía la llave que la regla pedía, y el 503 del almacén que reventaba en vez de declararse); smokes 100 % offline con fixtures SINTÉTICOS generados por código; contrato 1.14 | Proposed |
