...
8. `ls substrate_calibration/retrospectives/ 2>/dev/null` — read the latest self-critique + open `governance_queue.jsonl` (the RIL ledger). Seeds exist now; the full read-the-prepend reflex lands with the `retrospector` in Cycle 3 (ADR-0009).
...
- **Last updated:** 2026-06-11 (GWT v1.1 Cycle 1: source-of-truth + autoresearch entry).
...
- **GWT v1.1 deferred (Cycles 2–5, proposals):** RIL `retrospector` + `RIL_PROGRAM.md` charter + reactive auto-cap (Cycle 3, ADR-0009/0016) · noise-probe/EPS + effective frontier (Cycle 2) · PR-03..11 · the RAG/corpus/viz (rack) architecture is **OPEN** (exploration track; plan §A is reference-only, per the user).
...
- **Repo version:** GWT v1.1 — Cycle 1 applied on `feat/gwt-v1.1-cycle1`; Cycles 2–5 proposed.
