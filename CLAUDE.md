...
Tool Universe is invoked at one of four layers (see `skills/external/README.md` for layer selection):
...
- **Layer 0 — Workspace tool (ADR-0059):** the project's OWN tools in `.tooluniverse/tools/`, written stdlib-pure so they are importable by path with **no SDK and no MCP session**. This is the only layer that runs inside the webapp's server-side pipeline (`answer_pipeline._workspace_tool`), which is why `zfin_zebrafish` (native zebrafish phenotypes, the evidence tier the human-centric tools cannot give) and `pubmed_literature` (PubMed via NCBI E-utilities — ADR-0062 measured and rejected installing the SDK into the service container: the pinned 1.2.6 does not resolve on py3.12 and the latest pulls 173 packages) are first-class Path-B sources while Layers 1–3 remain agent-session-only. Populate as `tooluniverse-workspace: <tool-name>`. **A new workspace tool MUST stay stdlib-pure**: the moment it needs a dependency it stops being reachable from the pipeline and silently drops back to being a directive.
